...
class Robot:
    ...
    def is_object_detected_nearby(self):
        ...
        detection_distance = 2.0  # Distânacia de detecção em metros
        ...
        for i in range(0,self.y_limit, 20):  # Reduz a resolução para processamento mais rápido
            for j in range(0, w, 40):
                ...
                if detection_distance - threshold < d < detection_distance + threshold:
                    angle = np.arctan2(j-w //2, w // (2 *np.tan(np.deg2rad(60.0) / 2)))
    ...
    def dynamic_window(self, dt):
        ...
        if self.profundidade is not None:
            h,w = self.profundidade.shape
            y_indices = range (0,self.y_limit,20)
            x_indices = range (0,self.y_limit,20)
    ...
    def evaluate_trajectory(self, v, w, dt, goal):
        ...
        for ox,oy in self.obstacles_positions:
            distance_to_obstacle = np.linalg.norm([ox - x, oy - y])
            if distance_to_obstacle < self.safety_distance:
                total_cost += float('inf')
    ...
    def replan_velocity(self, dt, goal):
        ...
        if self.is_object_detected_nearby():
            rospy.loginfo("Obstáculo detectado. Avaliando direção de desvio.")
            left_cost = self.evaluate_trajectory(0,self.max_w,dt,goal)
            rigth_cost = self.evaluate_trajectory(0,-self.max_w,dt,goal)

            if left_cost < rigth_cost:
